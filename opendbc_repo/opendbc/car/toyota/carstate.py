  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    if self.CP.carFingerprint in TSS2_CAR:
      ...
      if self.CP.carFingerprint == CAR.TOYOTA_COROLLA_TSS2:
        # Unavailable LDA is still enabled; switching it off only clears LDA_UNAVAILABLE.
        hud = cp_cam.vl_all["LKAS_HUD"]
        for status, unavailable in zip(hud["LKAS_STATUS"], hud["LDA_UNAVAILABLE"], strict=True):
          enabled = bool(status or unavailable)
          if self.lkas_enabled is not None and enabled != self.lkas_enabled:
            buttonEvents.extend(create_button_events(1, 0, {1: ButtonType.lkas}) +
                                create_button_events(0, 1, {1: ButtonType.lkas}))
          self.lkas_enabled = enabled
      else:
